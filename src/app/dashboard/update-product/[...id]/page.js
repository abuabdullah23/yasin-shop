...
import { addProduct, getSingleProduct, updateProduct } from '@/api/products';
...
const UpdateProductPage = () => {
    ...
    const handleUpdateProduct = async (event) => {
        ...
        updateProduct(id, updateProductInfo).then((data) => {
            ...
            if (data.modifiedCount > 0) {
                ...
                toast.error('You can not change anything')
                setLoader(false)
            }
        });
    }
    ...
    return (
        <div>
            <div className='w-full p-4 shadow-xl border-2 border-green-400 rounded-md'>
                    ...
                    <form onSubmit={handleUpdateProduct}>
                        <div className='flex flex-col md:flex-row gap-4 w-full mb-4'>
                            <div className='flex flex-col w-full gap-1'>
                                ...
                                <input required className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600' name='name' type="text" defaultValue={product.name} placeholder='Edit product name' />
                            </div>
                            <div className='flex flex-col w-full gap-1'>
                                <label htmlFor="brand">Product brand</label>
                                <input required className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600' name='brand' type="text" defaultValue={product.brand} placeholder='Edit product brand' />
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col md:flex-row gap-4 w-full mb-4'>
                            ...
                            <div className='flex flex-col w-full gap-1'>
                                ...
                                <input required className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600' name='stock' type="number" defaultValue={product.stock} placeholder='stock' min={0} />
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col md:flex-row gap-4 w-full mb-4'>
                            <div className='flex flex-col w-full gap-1'>
                                ...
                                <input required className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600' name='price' type="number" min={0} defaultValue={product.price} placeholder='Price ৳' />
                            </div>
                            <div className='flex flex-col w-full gap-1'>
                                <label htmlFor="discount">discount</label>
                                <input className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600' name='discount' type="number" min={0} defaultValue={product.discount} placeholder='%discount%' />
                            </div>
                        </div>
                        ...
                        <div className='flex flex-col md:flex-row gap-4 w-full mb-4'>
                            <div className='flex flex-col w-full gap-1'>
                                ...
                                <textarea required onChange={handleAreaHeight} className='px-4 py-2 border border-slate-700 focus:border-indigo-500 outline-none bg-slate-100 rounded-md text-slate-600 overflow-hidden' name='description' type="text" min={0} defaultValue={product.description} placeholder='Description' />
                            </div>
                        </div>
                        ...
                        <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 xs:gap-4 md:gap-4 w-full mb-4'>
                            {
                                imageShow?.map((img, i) => <div className='h-[180px] relative border border-slate-300' key={i}>
                                    <label htmlFor={i}>
                                        <img className='w-full h-full rounded-sm object-cover' src={img} alt="category" />
                                    </label>
